...
void NSGA2::run(vector<Solution> P, int populationSize,
...
vector<vector<Solution>> NSGA2::fastNondominatedSort(vector<Solution> R) {
	...
	while (fronts.at(w).size() != 0) {
		...
		for (int i = 0; i < fronts.at(w).size(); i++) {
...
void NSGA2::crowdingDistanceAssignment(vector<Solution> front) {
	...
	for (int i = 0; i < this->numberOfObjectives; i++) {
		...
		front.at(front.begin()).distance = numeric_limits<float>::infinity();
		front.at(front.end()) = numeric_limits<float>::infinity();
